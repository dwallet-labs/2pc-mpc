...
pub type ConstrainedWitnessGroupElement<    const NUM_RANGE_CLAIMS: usize,
    const WITNESS_MASK_LIMBS: usize,> = self_product::GroupElement<
    NUM_RANGE_CLAIMS,
    power_of_two_moduli::GroupElement<WITNESS_MASK_LIMBS>,
>;
pub type ConstrainedWitnessValue<    const NUM_RANGE_CLAIMS: usize,
    const WITNESS_MASK_LIMBS: usize,> = group::Value<self_product::GroupElement<
    NUM_RANGE_CLAIMS,
    power_of_two_moduli::GroupElement<WITNESS_MASK_LIMBS>,
>>;
pub type ConstrainedWitnessPublicParameters<    const NUM_RANGE_CLAIMS: usize,
    const WITNESS_MASK_LIMBS: usize,> = group::PublicParameters<self_product::GroupElement<
    NUM_RANGE_CLAIMS,
    power_of_two_moduli::GroupElement<WITNESS_MASK_LIMBS>,
>>;
...
// But in the case of the witness mask, we actually pass it as WITNESS_MASK_LIMBS and it is of that size.
...
// or even you can say, it only knows the MessageSpaceGroupElement=EC-SCALAR in bulletproof of that commitment,
// we need somehow to call .commit() with WITNESS_MASK_LIMBS numbers.
// - yes, we don't use that commit later for .prove_range(). So we don't care that proof will fail.
// But, we do use it in group_homomorhpism(), and I'm afraid it will go through modulation for the witness mask there.
// yes, but this is coupling !!!! it assumes more knowledge on the range proof.
// ??? what group? we were always talking about the Paillier encryption
// yes, I remember, but that was for optimizing the range proof Pedersen - for proving it secure without safe primes.
// no no, even if the witness is one bit, I can't do this...
// because you are coupling! we are using generic range proofs... stop coupling!
// ????????/
// i am always talking about bulletproofs - I am talking about the commitment scheme of the range proof. This is bulletproof.
// But it is the range proof - bulletproof - that defines this commitment scheme. not the schnorr lagnuage.
// but this is again coupling
// noooooooo
// I dont care if its 64 bit, 32 bit or whatever. Even the fact you are thinking of how many bits they are, it is coupling!!!!
// witness size + challenge (computational security ) size + statistical security < order of the message space of the commitment scheme of the range proof??
// because we cannot go through modulation for the masked witness in the commitment shceme? what will happen if we do? why
// ok. So we have coupling but we know it is well defined. That's ok.
// lets speak on the specific parameters. we have witness = 64, challenge = 128, statistical = 64. => 256 > q.
// so should use computational = 127? or I can simply repeat the proof if it fails.
...
pub type RangeProofCommitmentScheme<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentScheme<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemePublicParameters<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemePublicParameters<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeMessageSpaceGroupElement<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeMessageSpaceGroupElement<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeMessageSpacePublicParameters<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeMessageSpacePublicParameters<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeMessageSpaceValue<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeMessageSpaceValue<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeRandomnessSpaceGroupElement<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeRandomnessSpaceGroupElement<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeRandomnessSpacePublicParameters<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeRandomnessSpacePublicParameters<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeRandomnessSpaceValue<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeRandomnessSpaceValue<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeCommitmentSpaceGroupElement<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeCommitmentSpaceGroupElement<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;


pub type RangeProofCommitmentSchemeCommitmentSpacePublicParameters<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeCommitmentSpacePublicParameters<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;

pub type RangeProofCommitmentSchemeCommitmentSpaceValue<
    const NUM_RANGE_CLAIMS: usize,
    const RANGE_CLAIM_LIMBS: usize, const WITNESS_MASK_LIMBS: usize, L> = range::CommitmentSchemeCommitmentSpaceValue<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, RangeProof<NUM_RANGE_CLAIMS, RANGE_CLAIM_LIMBS, WITNESS_MASK_LIMBS, L>>;
